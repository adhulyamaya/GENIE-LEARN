...
def filter_courses(user_skills, courses):
    ...
    for course in courses:
        print(f"\nEvaluating course: {course.title}")
        print(f"skills_required field: '{course.skills}'")

        skill_match = False

        if course.skills and course.skills.strip():
            for skill in skills:
                if skill.lower() in course.skills.lower():
                    skill_match = True
                    print(f"  ✅ Skill match found: {skill}")
            ...
            skill_match = True
            print(f"  ✅ No skills specified for course - considering as match")

        if not skill_match:
            for skill in skills:
                if skill.lower() in course.title.lower():
                    skill_match = True
                    print(f"  ✅ Course title contains skill: {skill}")
                    break

            if not skill_match:
                tech_keywords = [
                    'python', 'django', 'developer', 'engineer', 'programmer',
                    'software', 'web', 'fullstack', 'full stack', 'back end',
                    'front end', 'javascript', 'react', 'node'
                ]
        ...
        if skill_match:
            print(f"  ✅ Course '{course.title}' MATCHED all criteria!")
            filtered_courses.append(course)
        else:
            print(f"  ❌ Course '{course.title}' filtered out - skill_match: {skill_match}")

    return filtered_courses


def rank_course(user_skills, course, api_key):
    """Rank a course based on user skills using Groq LLM."""
    client = Groq(api_key=api_key)

    prompt = f"""
    i wants to know which course is best suited for them based on my skills.
    ...
    Based on the match between the skills of the user and course details, assign a score from 0 to 100.
    ...
    try:
        completion = client.chat.completions.create(
            messages=[
                {
                    "role": "system",
                    "content": "You want to know which courses are suitable based on skills. Always respond with valid JSON."
                },
                {
                    "role": "user",
                    "content": prompt
